...
The container is a persistent store for netidx values and bscript
formulas, a bit like a global pasteboard, or a nosql database. At
startup it reads and publishes all the data in it's database, and then
waits for user interaction. When an authorized user writes to one of
it's published values it updates that value in the database, and then
...
in. For example rel(-1) gets the path of the cell in the same row, but
1 column to the left. rel(1) goes 1 column to the right. rel(-1, 0)
...
will be deleted. If any of the specified cells does not exist in the
